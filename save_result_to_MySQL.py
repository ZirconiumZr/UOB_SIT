    ...
    with connection.cursor() as cursor:
       ...
       #from dataframe"output" to get the SD, STT and label results
        ...
        for i in range(0,len(SIT_result)-1):
            cursor.execute(sql2, (audio_id_1,SIT_result.index[i], SIT_result.starttime[i],SIT_result.endtime[i],SIT_result.duration[i],SIT_result.text[i],SIT_result.label[i],'/Users/ruiqianli/Desktop/UOB internship/Speaker_diarization',time.strftime("%d/%m/%Y"),time.strftime("%H:%M:%S")))
